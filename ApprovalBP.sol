// An ApprovalBP is a modification of the BurnablePayment contract.
// This replaces the worker-called "Commit" function with the payer-called "ApproveWorker" function.
// This allows a user without ETH to be registered as the recipient,
// given that they have an independent method of contacting the payer and requesting such an approval.
//
...
contract ApprovalBPFactory {
    ...
    function newBP(address payer, uint autoreleaseInterval, string title, string initialStatement)
    external
    payable
    returns (address)
    {
        //pass along any ether to the constructor
        address newBPAddr = (new ApprovalBP).value(msg.value)(payer, autoreleaseInterval, title, initialStatement);
        emit NewApprovalBP(newBPAddr, payer, msg.value, autoreleaseInterval, title, initialStatement);
...
contract ApprovalBP {
    ...
    //BP will start with a payer but not a recipient
    address public payer;
    address public recipient;

    address constant BURN_ADDRESS = 0x0;

    //Set to true if fundsRecovered is called (offers another method of detecting recalled BPs)
    ...
    event FundsAdded(uint amount); //The payer has added funds to the BP.
